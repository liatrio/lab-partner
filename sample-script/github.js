...
module.exports = function (controller) {
  controller.hears([/^add me$/], 'direct_mention', async (bot, message) => {
    ...
    confirmation.ask('Enter your Github username', async (username) => {

      const user  = await controller.plugins.github.getGithubUser(username);
      ...
      const repos = await controller.plugins.github.getUserRepositories(username);
      //for (var i = 0; i < repos.length; i++) {
      //  const repo  = repos[i].full_name.split("/")[1];
      //  controller.plugins.github.watchForNewCommits(username, repo, (commit) => {
      //    bot.say({
      //      blocks: [
      //        {
      //          type: "section",
